...
const categoryBorderClass: Record<string, string> = {
  prompt:    'cat-border-prompt',
  skill:     'cat-border-skill',
  hook:      'cat-border-hook',
  plugin:    'cat-border-plugin',
  technique: 'cat-border-technique',
  workflow:  'cat-border-workflow',
}
...
export default function LibraryCard({ item, index = 0 }: Props) {
  const borderClass = categoryBorderClass[item.category] || ''
  ...
  return (
    <motion.div
      ...
      className={`library-card ${borderClass}`}
      style={{
        height: '100%',
        borderRadius: 12,
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ color: 'var(--text-dim)', display: 'flex', alignItems: 'center', gap: 4 }}>
              {item.source_type === 'github' ? <GitHubIcon /> : <RedditIcon />}
              <span className="font-mono" style={{ fontSize: 10, color: 'var(--text-dim)' }}>
                {item.source_type === 'github' ? 'GitHub' : 'Reddit'}
              </span>
            </span>
